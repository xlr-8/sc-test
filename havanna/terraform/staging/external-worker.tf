...
module "external-worker" {
  ...
  bastion_sg_allow         = data.terraform_remote_state.infrastructure.outputs.staging_bastion_sg_allow
  ...
  metrics_sg_allow        = data.terraform_remote_state.prometheus.outputs.staging_metrics_sg_allow

  #. public_subnets_ids (required, array):
  #+ Amazon public subnets IDs on which create each components.
  public_subnets_ids      = data.terraform_remote_state.infrastructure.outputs.staging_public_subnets
