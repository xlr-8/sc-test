...
module "external-worker" {
  ...
  bastion_sg_allow         = data.terraform_remote_state.infrastructure.outputs.dev_bastion_sg_allow
  ...
  metrics_sg_allow        = data.terraform_remote_state.prometheus.outputs.dev_metrics_sg_allow

  #. public_subnets_ids (required, array):
  #+ Amazon public subnets IDs on which create each components.
  public_subnets_ids      = data.terraform_remote_state.infrastructure.outputs.dev_public_subnets

  #. vpc_id (required):
  #+ Amazon VPC id on which create each components.
  vpc_id                  = data.terraform_remote_state.infrastructure.outputs.dev_vpc_id
